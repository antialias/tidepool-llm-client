...
import { DEFAULT_AI_FALLBACK_POLICY, DEFAULT_AI_RETRY_POLICY, parseAiModelReference, } from "./operations.js";
import { LLMApiError, ProviderNotConfiguredError, } from "./types.js";
import { OpenAIDslProvider } from "./providers/openai-dsl.js";
const TextResponseSchema = z.object({
    text: z.string(),
});
const FALLBACK_SAFE_KINDS = [
    "provider_rate_limited",
    "provider_unavailable",
    "timeout",
];
const DEFAULT_PROVIDER_FACTORIES = Object.freeze({
...
export class AiClient {
    constructor(config, currentScope = {}) {
        this.config = config;
    ...
    async object(request) {
        ...
        const envelope = await this.createEnvelope("text-object", request, jsonSchema, prompt);
        return this.runWithPolicy({
            envelope,
            fallbackAllowed: true,
            execute: (operationEnvelope, attempt, isFallback) => this.executeObjectAttempt(operationEnvelope, attempt, isFallback),
        });
    }
    async text(request) {
        const prompt = this.normalizePrompt(request);
        ...
        const envelope = await this.createEnvelope("text", request, jsonSchema, prompt);
        return this.runWithPolicy({
            envelope,
            fallbackAllowed: true,
            execute: (operationEnvelope, attempt, isFallback) => this.executeTextAttempt(operationEnvelope, attempt, isFallback),
        });
    }
    async streamObject(request) {
        const prompt = this.normalizePrompt(request);
        const jsonSchema = z.toJSONSchema(request.schema, {
            unrepresentable: "any",
        });
        const envelope = await this.createEnvelope("text-stream", request, jsonSchema, prompt);
        return this.executeStreamObject(envelope);
    }
    async embed(request) {
        const envelope = await this.createEnvelope("embedding", request);
        return this.runWithPolicy({
            envelope,
            fallbackAllowed: false,
            execute: (operationEnvelope, attempt, isFallback) => this.executeEmbedAttempt(operationEnvelope, attempt, isFallback),
        });
    }
    async imageEdit(request) {
        const envelope = await this.createEnvelope("image-edit", request);
        const providerConfig = getProviderConfig({ providers: this.config.providers, defaultProvider: envelope.resolvedModel.provider, defaultMaxRetries: 0 }, envelope.resolvedModel.provider);
        ...
        if (maxN && requestedN > maxN) {
            ...
            for (let i = 0; i < requestedN; i += maxN) {
                ...
                const subRequest = { ...request, n: subN };
                const subEnvelope = await this.createEnvelope("image-edit", subRequest);
                subResults.push(await this.runWithPolicy({
                    envelope: subEnvelope,
                    fallbackAllowed: true,
                    execute: (operationEnvelope, attempt, isFallback) => this.executeImageEditAttempt(operationEnvelope, attempt, isFallback),
                }));
            }
    ...
    async createEnvelope(operation, request, jsonSchema, prompt) {
        ...
        const fallbackPolicy = requestFallbackPolicy(request) ??
    ...
    async runWithPolicy(options) {
        const attempts = [];
        const primaryResult = await this.tryResolvedModel(options, attempts, false);
        if (primaryResult.ok) {
            return withAttempts(primaryResult.result, attempts);
        }
        const fallbackResult = await this.tryFallbacks(options, attempts, primaryResult.error);
        if (fallbackResult.ok) {
            return withAttempts(fallbackResult.result, attempts);
        }
        throw fallbackResult.error;
    }
    async tryResolvedModel(options, attempts, isFallback) {
        const maxAttempts = options.envelope.retry.attempts + 1;
        let lastError;
        let validationFeedback;
        for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
            const attemptEnvelope = withValidationFeedback(options.envelope, validationFeedback);
            attempts.push({
                attempt,
            ...
            try {
                const result = await this.runAttemptWithMiddleware(attemptEnvelope, attempt, () => options.execute(attemptEnvelope, attempt, isFallback));
    ...
    async tryFallbacks(options, attempts, primaryError) {
    ...
    async executeObjectAttempt(envelope, attempt, isFallback) {
        const response = await this.callStructuredProvider(envelope, envelope.request.schema, attempt, isFallback, envelope.validationFeedback);
        ...
        if (!parseResult.success) {
            throw new AiError({
                ...
                retryable: true,
                userVisible: false,
                validationFeedback: validationFeedbackFromZod(response.response.content, parseResult.error),
                ...this.errorContext(envelope),
            });
        }
        const result = {
    ...
    async executeTextAttempt(envelope, attempt, isFallback) {
        const response = await this.callStructuredProvider(envelope, TextResponseSchema, attempt, isFallback, envelope.validationFeedback);
        if (response.validationFeedback) {
            throw new AiError({
                kind: "schema_validation_failed",
    ...
    async executeEmbedAttempt(envelope, attempt, isFallback) {
        ...
        const request = {
            ...
            forceReal: envelope.request.forceReal ??
    ...
    async executeImageEditAttempt(envelope, attempt, isFallback) {
        const provider = this.getOrCreateProvider(envelope.resolvedModel.provider);
    ...
    async executeStreamObject(envelope) {
        const attempt = 1;
        const result = {
    ...
    async *createProviderObjectStream(envelope, attempt) {
        const provider = this.getOrCreateProvider(envelope.resolvedModel.provider);
    ...
    async callStructuredProvider(envelope, schema, attempt, _isFallback, validationFeedback) {
        const provider = this.getOrCreateProvider(envelope.resolvedModel.provider);
        const providerRequest = this.buildProviderRequest(envelope, validationFeedback);
        await this.emitBeforeProviderCall(envelope, attempt);
        const startedAt = Date.now();
        try {
            const pool = this.config.concurrency?.forProvider(envelope.resolvedModel.provider);
    ...
    async runAttemptWithMiddleware(envelope, attempt, execute) {
    ...
    buildProviderRequest(envelope, validationFeedback) {
        const isNativeOpenAI = envelope.resolvedModel.provider === "openai";
        const reasoningEffort = isNativeOpenAI && "reasoningEffort" in envelope.request
            ? envelope.request.reasoningEffort
            : undefined;
        return {
            endpoint: envelope.resolvedModel.provider.toLowerCase() === "anthropic"
                ? "/messages"
                : "/chat/completions",
            request: {
                prompt: envelope.prompt,
                images: "images" in envelope.request && envelope.request.images
    ...
    assertIntentCanRunOperation(intent, spec, operation) {
        if (!spec)
            return;
        ...
        const missingCapabilities = spec.requiredCapabilities?.filter((capability) => OPERATION_LEVEL_CAPABILITIES.has(capability) &&
    ...
    resolveModel(request, intentSpec) {
        ...
        const intentModel = intentSpec?.defaultModel
            ...
            : undefined;
        const modelCandidate = callModel?.model ??
            scopedModel?.model ??
            intentModel?.model ??
            this.config.defaultModel;
        const providerCandidate = request.provider ??
            callModel?.provider ??
        ...
        return {
            provider,
            model,
            source: callModel?.model || request.provider
                ? "call"
    ...
    resolveFallbackModel(candidate) {
    ...
    getOrCreateProvider(providerName) {
        ...
        const factory = this.config.providerFactories?.[normalizedProvider] ??
    ...
    requireProviderConfig(providerName) {
    ...
    hooks() {
        return [
            ...
            ...(this.config.plugins?.flatMap((plugin) => plugin.hooks ? [plugin.hooks] : []) ?? []),
        ];
    }
    async emitBeforeResolve(context) {
        for (const hooks of this.hooks()) {
            await hooks.beforeResolve?.(context);
        }
    }
    async emitBeforeProviderCall(envelope, attempt) {
        const context = this.lifecycleContext(envelope, attempt);
        for (const hooks of this.hooks()) {
            await hooks.beforeProviderCall?.(context);
        }
    }
    async emitAfterSuccess(envelope, attempt, result) {
        const context = this.lifecycleContext(envelope, attempt);
        for (const hooks of this.hooks()) {
            await hooks.afterSuccess?.(context, result);
        }
    }
    async emitAfterError(envelope, attempt, error) {
        const context = this.lifecycleContext(envelope, attempt);
        for (const hooks of this.hooks()) {
            await hooks.afterError?.(context, error);
        }
    }
    async emitAfterRetry(envelope, attempt, error) {
        const context = this.lifecycleContext(envelope, attempt);
        for (const hooks of this.hooks()) {
            await hooks.afterRetry?.(context, error);
        }
    }
    async emitFallbackDecision(decision) {
        for (const hooks of this.hooks()) {
            await hooks.afterFallbackDecision?.(decision);
        }
    }
    lifecycleContext(envelope, attempt) {
        return {
            registry: this.config.registry,
            scope: this.currentScope,
            envelope,
...
function mergeScope(parent, child) {
    return {
        ...parent,
        ...child,
        usageContext: parent.usageContext && child.usageContext
...
function requestCallSite(request) {
    return request.callSite;
}
function requestRetry(request) {
    return request.retry;
}
function requestFallbackPolicy(request) {
    return request.fallbackPolicy;
}
function requestUsageContext(request) {
    return request.usageContext;
}
function requireJsonSchema(jsonSchema) {
    if (!jsonSchema) {
        throw new AiError({
            kind: "invalid_request",
...
const OPERATION_KIND_CAPABILITIES = {
...
const OPERATION_LEVEL_CAPABILITIES = new Set(Object.values(OPERATION_KIND_CAPABILITIES).flat());
function shouldRetry(error, policy, attempt) {
    return attempt <= policy.attempts && policy.on.includes(error.kind);
}
function canFallback(error, policy) {
    return (policy.mode === "ordered" &&
...
function validationFeedbackFromZod(received, error) {
 ...
 */
function mergeImageEditResults(subResults, errorContext) {
    ...
    for (const key of Object.keys(last)) {
        if (key === "images" || key === "attempts")
            continue;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const sample = last[key];
        if (!Array.isArray(sample))
            continue;
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        merged[key] = subResults.flatMap((r) => r[key]);
    }
    return merged;
}
function withValidationFeedback(envelope, validationFeedback) {
...
function hasEmbedding(provider) {
    return typeof provider.embed === "function";
}
function hasStreaming(provider) {
    return typeof provider.streamObject === "function";
}
function hasImageEdit(provider) {
