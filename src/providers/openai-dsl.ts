...
export class OpenAIDslProvider
  implements LLMProvider, AiStreamingProvider, AiEmbeddingProvider, AiImageProvider
{
  ...
  async imageEdit(
    ...
    formData.append(
      "prompt",
      `Using the reference images for visual consistency (character appearance, art style, color palette), generate the following scene:\n\n${request.prompt}`,
    );
    ...
    return {
      ...
      usage: {
        ...
        imageOutputTokens:
          raw?.output_tokens_details?.image_tokens ?? raw?.output_tokens,
        raw,
      },
      model: request.model,
      responseId: data.id,
