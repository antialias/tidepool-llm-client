...
export type AiResolutionSource =
  | "call"
  | "scope"
  | "user"
  | "sitewide"
...
export class AiError<TKind extends AiErrorKind = AiErrorKind> extends Error {
  readonly kind: TKind;
...
export function isAiError<TKind extends AiErrorKind>(
  error: unknown,
  kind: TKind,
): error is AiError<TKind>;
export function isAiError<TKind extends AiErrorKind>(
  error: unknown,
  kind?: TKind,
): error is AiError<TKind> {
...
function mapApiError(error: LLMApiError, context: AiErrorContext): AiError {
...
function enrichAiError(error: AiError, context: AiErrorContext): AiError {
  if (
    error.intent === context.intent &&
