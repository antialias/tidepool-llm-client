...
Sometimes you just need a string. The `.text()` method skips the schema:

```typescript
const tagline = await ai.text({
...
Notice that `model` can be passed directly on the request. This overrides the intent's default (or the client's default) for just this call. The format is `"provider/model"`.
