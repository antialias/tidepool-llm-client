...
Migrating to `@tidepool/llm-client`'s `AiClient` / `createLlmClient` API from raw provider SDKs or the legacy `LLMClient` class. All three paths converge on the same target: `createLlmClient()` with an intent registry.
...
## From legacy `LLMClient`

### API mapping

| Legacy `LLMClient` | New `AiClient` |
|---|---|
| `new LLMClient()` | `createLlmClient({ registry, providers })` |
| `llm.call({ prompt, schema })` | `ai.object({ prompt, schema })` |
| `llm.stream({ prompt, schema })` | `ai.streamObject({ prompt, schema })` |
| `llm.embed({ input })` | `ai.embed({ input })` |
| `llm.vision({ prompt, schema, images })` | `ai.object({ prompt, schema, images })` |
| `request.maxRetries: 3` | `retry: { attempts: 3, on: [...] }` on intent or request |
| `request.onProgress` | Hooks via `plugins` or `hooks` on the client config |
| `response.data` | `result.output` |
| `llm.with(middleware)` | `plugins` array in client config |
| `llm.withRequestDefaults({ forceReal })` | `ai.scope({ forceReal })` |

### Error mapping

| Legacy error class | `AiError` kind |
|---|---|
| `LLMValidationError` | `"schema_validation_failed"` |
| `LLMApiError` (429) | `"provider_rate_limited"` |
| `LLMApiError` (5xx) | `"provider_unavailable"` |
| `LLMApiError` (4xx) | `"invalid_request"` |
| `LLMTimeoutError` | `"timeout"` |
| `LLMNetworkError` | `"provider_unavailable"` |
| `LLMContentFilterError` | `"provider_refused"` |
| `LLMTruncationError` | `"context_too_large"` |
| `LLMJsonParseError` | `"provider_bad_response"` |
| `ProviderNotConfiguredError` | `"provider_not_configured"` |

**Before:**
```typescript
import { LLMClient, LLMValidationError, LLMApiError } from "@tidepool/llm-client";
const llm = new LLMClient({ defaultModel: "gpt-4.1" });

try {
  const response = await llm.call({ prompt: "Extract entities...", schema: EntitySchema, maxRetries: 2 });
  console.log(response.data);
} catch (err) {
  if (err instanceof LLMValidationError) { /* ... */ }
  else if (err instanceof LLMApiError && err.statusCode === 429) { /* ... */ }
}
```

**After:**
```typescript
import { createLlmClient, defineIntentRegistry, isAiError, loadConfigFromEnv } from "@tidepool/llm-client";

const registry = defineIntentRegistry({
  entityExtraction: {
    kind: "text-object",
    defaultModel: "openai/gpt-4.1",
    retry: { attempts: 2, on: ["schema_validation_failed", "provider_rate_limited"] },
  },
});
const ai = createLlmClient({ registry, providers: loadConfigFromEnv().providers });

try {
  const result = await ai.intent("entityExtraction").object({ prompt: "Extract entities...", schema: EntitySchema });
  console.log(result.output);
} catch (err) {
  if (isAiError(err, "schema_validation_failed")) { /* ... */ }
  else if (isAiError(err, "provider_rate_limited")) { /* ... */ }
}
```
...
**1. List call sites.** Find every `LLMClient` instantiation or raw SDK call. Note the model, provider, retry count, and schema.
...
**6. Add a usage recorder** (optional). Pass `usageRecorder` to the client config -- same `LLMUsageEvent` shape as the legacy client.
