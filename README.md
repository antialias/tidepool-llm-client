...
ESM-only. Node >= 18. Published via semantic-release to the
[Tidepool Gitea npm registry](https://git.dev.abaci.one).

## Status

Internal shared dependency between Tidepool and Abaci. The API surface is
stabilizing — see the `#500` epic for the design rationale. The canonical
production integration is
[`apps/portal/src/lib/ai/gateway.ts`](../../apps/portal/src/lib/ai/gateway.ts).
